...
function brokenIndex(rung: RecallRung, because: string): RecallIndex {
  return {
    ...
    sync: (): IndexSyncReport => ({ mined: 0, skipped: 0, removed: 0 }),
    search: (): readonly RecallHit[] | null => null
  }
}

describe('Library.corpus', () => {
  it('gathers memory, archive and the knowledge shelf, in a stable order', () => {
  ...
  it('answers nothing — not an error — for a company with no agents yet', () => {
    const { library } = rig()
    expect(library.corpus()).toEqual([])
  })

  it('carries the stat facts the mtime gate compares', () => {
    const { library, agoraRoot } = rig()
    seedCorpus(library, agoraRoot)
...
describe('the ladder is visible at every rung (invariant §7)', () => {
  it('answers on grep with a reason when no index is configured', () => {
    const { library, agoraRoot } = rig()
    seedCorpus(library, agoraRoot)
  ...
  it('answers on fts, with nothing to report, when the index is up', () => {
    ...
    expect(library.recall('flaky checkout').degraded).toBeNull()
  })

  it('names the rung it could not use when an index is absent', () => {
  ...
  it('steps past a rung that is up but fails the query, and says which', () => {
    const { library, agoraRoot } = rig({
  ...
  it('reaches grep even when every rung above it is broken — the floor holds', () => {
    const { library, agoraRoot } = rig({
  ...
  for (const { name, indexes } of rungs) {
    describe(name, () => {
      ...
      for (const item of known) {
        it(`"${item.query}" finds "${item.expect}"`, () => {
          const { library, agoraRoot } = rig({ indexes })
          seedCorpus(library, agoraRoot)
          library.reindex()
          const answer = library.recall(item.query, item.scope)
      ...
      it('answers nothing for something the company does not know', () => {
        const { library, agoraRoot } = rig({ indexes })
        seedCorpus(library, agoraRoot)
        library.reindex()
        expect(library.recall('kubernetes helm chart').hits).toEqual([])
      })

      it('keeps a scoped query inside its scope', () => {
        const { library, agoraRoot } = rig({ indexes })
        seedCorpus(library, agoraRoot)
        library.reindex()
        const answer = library.recall('staging', 'agent.iris')
        expect(answer.hits.length).toBeGreaterThan(0)
...
describe('mtime-gated incremental indexing (ADR-0006)', () => {
  it('mines every document once, then nothing until one changes', () => {
    const fts = new FtsIndex({ store: new MemoryFtsStore() })
    const { library, agoraRoot } = rig({ indexes: [fts] })
    seedCorpus(library, agoraRoot)

    const first = library.reindex().get('fts')
    expect(first).toEqual({ mined: 4, skipped: 0, removed: 0 })

    const second = library.reindex().get('fts')
    expect(second).toEqual({ mined: 0, skipped: 4, removed: 0 })
  })

  it('re-mines exactly the document that changed', () => {
    const fts = new FtsIndex({ store: new MemoryFtsStore() })
    const { library, agoraRoot } = rig({ indexes: [fts] })
    seedCorpus(library, agoraRoot)
    library.reindex()
    ...
    const report = library.reindex().get('fts')
    expect(report).toEqual({ mined: 1, skipped: 3, removed: 0 })
    expect(library.recall('seeded database').hits[0]?.scope).toBe('agent.iris')
  })

  it('forgets a document that has left the corpus — the index is derived state', () => {
    const fts = new FtsIndex({ store: new MemoryFtsStore() })
    const { library, agoraRoot } = rig({ indexes: [fts] })
    seedCorpus(library, agoraRoot)
    library.reindex()
    ...
    expect(library.reindex().get('fts')).toEqual({ mined: 0, skipped: 3, removed: 1 })
    expect(library.recall('promote production').hits).toEqual([])
  })

  it('does not sync a rung that is not available', () => {
    const absent = new FtsIndex({ store: null, because: 'no index' })
    const { library, agoraRoot } = rig({ indexes: [absent] })
    seedCorpus(library, agoraRoot)
    expect(library.reindex().size).toBe(0)
  })
})

describe('FtsIndex.search', () => {
  it('finds nothing to search for in a query of pure punctuation', () => {
    const fts = new FtsIndex({ store: new MemoryFtsStore() })
    expect(fts.search('?!', null, 5)).toEqual([])
  })

  it('reports itself unavailable, with a reason, when it has no store', () => {
    ...
    expect(fts.search('anything', null, 5)).toBeNull()
    expect(fts.sync([] as readonly IndexableDoc[])).toEqual({ mined: 0, skipped: 0, removed: 0 })
  })
})
