...
### Failures seen alongside this work

At the time, `test/shared/cost.test.ts`, `npm run typecheck` (a missing
`limezu.emotes.json`) and a Windows `EPERM`/`EBUSY` flake in temp-directory
teardown all failed here. The teardown flake was root-caused separately
(`docs/implementations/2026-09-01-flaky-temp-dir-teardown.md`); the other two
turned out to be real defects rather than facts about this machine.
